...
from app.services.expense_services import category_services
...
async def get_expenses_by_category(category: str):
    expenses = category_services.get_expenses_by_category(category)
    return expenses
...
async def get_expenses_by_category_and_month(category: str, year: int, month: int):
    expenses = category_services.get_expenses_by_category_and_month(
        category, year, month
    )
    return expenses
...
async def get_expenses_by_category_and_year(category: str, year: int):
    expenses = category_services.get_expenses_by_category_and_year(category, year)
    return expenses
...
async def get_expenses_by_category_and_month_range(
    ...
    expenses = category_services.get_expenses_by_category_and_month_range(
